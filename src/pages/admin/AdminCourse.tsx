...
export default function AdminCourse() {
  ...
  return (
    <>
      <div className="mb-2 text-sm text-ink/50"><Link to="/admin" className="hover:underline">Courses</Link> / {course.title}</div>
      <PageTitle title={course.title} subtitle={course.description}
        actions={<>
          ...
          <button className="btn-accent" onClick={() => setCreating(true)}><Plus size={16} /> New module</button>
        </>} />
      {modules.length === 0 && <Empty>No modules yet. A module holds content (images, PDFs, videos, text), an optional quiz and optional homework.</Empty>}
            ...
            <li key={m.id} className="card flex items-center gap-3 !py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-2 text-xs text-ink/50">
                  <span>{x?.items ?? 0} content item{x?.items === 1 ? '' : 's'}</span>
                  {x?.quiz && <span className="text-mint">· quiz</span>}
                  {x?.hw && <span className="text-accent">· homework</span>}
                  {!m.published && <span className="badge bg-ink/5 text-ink/50">hidden</span>}
                </div>
              </div>
              <div className="flex items-center gap-1">
                ...
                <button className="btn-ghost !px-2" onClick={() => togglePublish(m)} title={m.published ? 'Hide from students' : 'Show to students'}>{m.published ? <EyeOff size={14} /> : <Eye size={14} />}</button>
                <button className="btn-danger !px-2" onClick={() => remove(m)}><Trash2 size={14} /></button>
                <Link to={`/admin/module/${m.id}`} className="btn-primary !px-3">Edit <ChevronRight size={14} /></Link>
              </div>
            </li>
          )
        })}
      ...
      {creating && (
        <Modal title="New module" onClose={() => setCreating(false)}>
          <QuickForm submitLabel="Create module" placeholder="Module title" onSubmit={async (v) => {
      ...
      {editing && (
        <Modal title="Edit course" onClose={() => setEditing(false)}>
          <QuickForm submitLabel="Save" initial={{ title: course.title, description: course.description ?? '' }} onSubmit={async (v) => {
